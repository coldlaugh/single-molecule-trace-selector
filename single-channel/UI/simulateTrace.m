...
for experiment = 1 : 100
    ...
    noise = random('norm',0.15,0.03)*totcounts; % Noise level (currently a constant multiple of totcounts, and independent of channel)
    ...
    Pmat = random('norm',0.05,0.02,Ns,Ns);
    ...
    acceptor = acceptor_true + normrnd(10,noise,frames,1);
    donor = donor_true + 0.5 * normrnd(10,noise,frames,1);
    selection = (acceptor_true>0) & (donor_true>0);
    FRET = acceptor./(donor+acceptor);

    if save_traces==1
           waitbar(experiment/100,f,'Simulating traces');
           path = '/Users/leyou/Desktop/FRET/beta2.0/trainingData/set5/goodMol/';
           ...
           symbols = ['a':'z' 'A':'Z' '0':'9'];
             ...
             st = symbols (nums);
           loc = strcat(path,'simulatedTrace_',st,'.mat');
           save(loc,'trace');
    end
