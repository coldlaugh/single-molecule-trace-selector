...
function plot_trace(trace, segLabel)
    ...
    p = plot(frame, trace(1,:),'--','Color',[0 0.5 0],'LineWidth',1);
    p.Color(4) = 0.1;
    p = plot(frame, trace(2,:),'r--','LineWidth',1);
    p.Color(4) = 0.1;
    p = plot(frame(segLabel), trace(1,segLabel),'--','Color',[0 0.5 0],'LineWidth',1);
    p.Color(4) = 0.8;
    p = plot(frame(segLabel), trace(2,segLabel),'r--','LineWidth',1);
    p.Color(4) = 0.8;
%     xlim([0 2000])
    pause
end
