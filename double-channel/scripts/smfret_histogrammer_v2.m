...
[fname,fpath,ffiltin] = uigetfile('*.dat', 'MultiSelect', 'on');
...
numsegments=size(fname,2);

for i=1:numsegments
   fullname = strcat(fpath,fname{i}(1,:));
   segcell{i}=load(fullname);
   segcell{i}=cat(2,segcell{i},segcell{i}(:,3)./(segcell{i}(:,2)+segcell{i}(:,3)));
end
