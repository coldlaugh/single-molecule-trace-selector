...
outputDir = '../data/serial/simulated';
...
noise = 0.1*totcounts; % Noise level (currently a constant multiple of totcounts, and independent of channel)
...
% Pmat = random('Gamma',5,0.01,Ns,Ns);
Pmat = random('Exponential',0.05,Ns,Ns);
...
% end
% if msg == 1
% %         disp('Probabilities do not add up to 100%!  Re-scaling probabilities to add up to 100%.');
% end
...
k = unidrnd(Ns); % Select random initial state
...
acceptor = acceptor_true + normrnd(0,noise,frames,1);
donor = donor_true + normrnd(0,noise,frames,1);
label = (donor_true > 0) & (acceptor_true > 0);
...
save([tempname(outputDir),'.mat'],'data','-mat');
if plot_traces==1
figure(1)
plot(acceptor,'r-');
hold on
plot(donor,'g-');
plot(totcounts * label,'m-');
hold off
figure(2)
plot(FRET,'k-');
ylim([-0.25 1.25]);
